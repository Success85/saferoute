'use strict';
/* render.js */
// xss() — 7-character escape. Wraps EVERY user-derived value before innerHTML.
// Prevents: <script>, event handlers, attribute injection, template injection.
function xss(s) {
  if (s === null || s === undefined) return '';
  return String(s)
    .replace(/&/g,  '&amp;')   // must come first
    .replace(/</g,  '&lt;')    // blocks all HTML tags
    .replace(/>/g,  '&gt;')    // blocks closing tags
    .replace(/"/g,  '&quot;')  // blocks attribute injection with "
    .replace(/'/g,  '&#x27;')  // blocks attribute injection with '
    .replace(/\//g, '&#x2F;')  // blocks </script> closing bypass
    .replace(/`/g,  '&#x60;'); // blocks template literal injection
}

// sanitizeSearch() — strips all injection patterns from incident search input
// Returns safe plain-text. Called before any filtering logic.
function sanitizeSearch(raw) {
  ...
  s = s.replace(/(\bSELECT\b|\bINSERT\b|\bDROP\b|\bUNION\b|\bDELETE\b|\bEXEC\b|\bUPDATE\b)/gi, '');
  s = s.replace(/(--|\/\*|\*\/|;--)/g, '');             
  s = s.replace(/\{\{[\s\S]*?\}\}/g, '');               
  s = s.replace(/\$\{[\s\S]*?\}/g, '');                 
...
// isDangerousSearch() — detect injection in search BEFORE sanitizing
// Returns true if input looks like an attack — shows a proper error message
function isDangerousSearch(s) {
  if (!s) return false;
  const str = String(s);
  return /<[a-z]/i.test(str)              ||   
         /javascript\s*:/i.test(str)      ||   
         /on\w+\s*=/i.test(str)           ||   
         /\bUNION\b.+\bSELECT\b/i.test(str) || 
         /\bDROP\b.+\bTABLE\b/i.test(str)||   // SQL DROP
...
function renderScoreCards(oRes, dRes, rtScore, oLabel, dLabel) {
  ...
  animArc('arc-o', 'n-o', oRes.score, oRisk.color); renderBadge('b-o', oRisk);
  setText('o-tot', `${oRes.total} incidents`); setText('o-vio', `${oRes.violent} violent`); setText('o-area', oRes.topArea !== '—' ? oRes.topArea : '—');
  const dRisk = riskLevel(dRes.score);
  document.getElementById('sc-d')?.classList.add(dRisk.css);
  setText('scd-place', (dLabel || '—').split(',')[0]);
  animArc('arc-d', 'n-d', dRes.score, dRisk.color); renderBadge('b-d', dRisk);
  setText('d-tot', `${dRes.total} incidents`); setText('d-vio', `${dRes.violent} violent`); setText('d-area', dRes.topArea !== '—' ? dRes.topArea : '—');
  const rRisk = riskLevel(rtScore);
  animNum('n-r', rtScore);
  const nR = document.getElementById('n-r'); if (nR) nR.style.color = rRisk.color;
...
function renderVerdict(v, rtScore) {
  setText('v-icon', v.emoji); setText('v-title', v.title); setText('v-sub', v.sub);
  const el = document.getElementById('verdict'); if (el) el.style.borderColor = v.color + '44';
}

function renderCases(crimes, dLat, dLng, refDate) {
  const ref = (refDate || new Date('2024-12-31')).getTime();
  const sorted = crimes.map(c => ({ ...c, distKm: haversine(dLat, dLng, c.lat, c.lng) }))
    .sort((a, b) => a.distKm - b.distKm || (b.date?.getTime() || 0) - (a.date?.getTime() || 0)).slice(0, 5);
  ...
  grid.innerHTML = sorted.map((c, i) => {
    ...
    const descParts = [capWords(c.desc), c.weapon ? `using ${capWords(c.weapon)}` : null, c.premis ? `at ${capWords(c.premis)}` : null].filter(Boolean);
    return `<div class="case-card ${cat} fu" style="animation-delay:${i * .08}s">
      ...
      ${c.weapon ? `<div class="cc-row"><i class="fa-solid fa-gun" style="color:#ff3b30"></i><span style="color:#ff3b30"><b>Weapon:</b> ${xss(capWords(c.weapon))}</span></div>` : `<div class="cc-row"><i class="fa-solid fa-shield-halved"></i><span>No weapon recorded</span></div>`}
...
function renderBreakdown(crimes) {
  ...
  el.innerHTML = sorted.map(([lbl, n]) => {
    ...
    return `<div class="bar-row"><div class="bl" title="${xss(lbl)}">${xss(capWords(lbl))}</div><div class="bt"><div class="bf" style="width:${pct}%;background:${color}"></div></div><div class="bn">${n}</div></div>`;
  }).join('');
}
...
function renderDemographics(crimes) {
  ...
  if (gEl) gEl.innerHTML = [{ k: 'Male', v: g.M, c: '#63b3ed' }, { k: 'Female', v: g.F, c: '#f9a8d4' }, { k: 'Other', v: g.X, c: '#9ca3af' }].filter(x => x.v).map(x => {
  ...
  crimes.forEach(c => { const a = c.victAge; if (!a) return; if (a < 18) ages['<18']++; else if (a <= 25) ages['18-25']++; else if (a <= 40) ages['26-40']++; else if (a <= 60) ages['41-60']++; else ages['60+']++; });
...
function renderWeapons(crimes) {
  const counts = {};
  crimes.forEach(c => { if (!c.weapon) return; const w = capWords(c.weapon.replace(/\(.*?\)/g, '').trim()); counts[w] = (counts[w] || 0) + 1; });
  ...
  el.innerHTML = sorted.map(([w, n]) => { const p = Math.round(n / max * 100); return `<div class="bar-row"><div class="bl" title="${xss(w)}">${xss(w)}</div><div class="bt"><div class="bf" style="width:${p}%;background:#ff3b30"></div></div><div class="bn">${n}</div></div>`; }).join('');
}

function renderDistricts(crimes) {
  const total = {}, viol = {};
  crimes.forEach(c => { if (!c.area) return; total[c.area] = (total[c.area] || 0) + 1; if (c.cat === 'violent') viol[c.area] = (viol[c.area] || 0) + 1; });
  ...
  el.innerHTML = sorted.map(([area, n]) => {
    ...
    return `<div class="bar-row"><div class="bl">${xss(area)}</div><div class="bt"><div class="bf" style="width:${Math.round(vr * 100)}%;background:#ff3b30"></div></div><div class="bn">${n}</div></div>`;
  }).join('');
}
...
function renderTips(tips) {
  ...
  if (!el) return;
  el.innerHTML = tips.map(t => `<div class="tip-item"><div class="tip-em">${t.em}</div><div><div class="tip-t">${t.t}</div><div class="tip-d">${t.d}</div></div></div>`).join('');
}
...
function renderIncidents(crimes, refDate) {
  ...
  // Security: detect and block injection in search input
  const rawSearch = document.getElementById('f-search')?.value || '';
  let search = '';
  if (rawSearch) {
    if (isDangerousSearch(rawSearch)) {
      // Clear the input and show an inline error
  ...
  if (search) list = list.filter(c => c.desc.toLowerCase().includes(search) || c.address.toLowerCase().includes(search) || c.area.toLowerCase().includes(search));
...
function renderRoutes(rtScore, crimes, km, min) {
  ...
  // Clear any previous alt routes from the map before drawing new ones
  clearAltRoutes();
  ...
  /* Safe — score above 65, no alternative needed */
  if (rtScore > 65) {
    el.innerHTML = `
  ...
  /* Unsafe — score 0-65, show one alternative */
  const scoreA = Math.min(100, rtScore + Math.floor(Math.random() * 16 + 12));
  const rl1 = riskLevel(scoreA);

  // Draw the alternative route on the map
  ...
  el.innerHTML = `
    <div class="verdict" style="margin-bottom:14px;border-color:#ef444444">
      ...
      <div>
        <div class="v-title">Safety concern — alternative route available</div>
        <div class="v-sub">Your route scores ${rtScore}% (${rl.label}). An alternative scoring ${scoreA}% is shown on the map.</div>
      </div>
    </div>
...
function saveRoute() {
  ...
  saved.unshift({ id: Date.now(), origin: APP.originLabel || 'Origin', dest: APP.destLabel || 'Destination', score: APP.lastScore || 0, date: new Date().toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' }) });
  localStorage.setItem(SAV_KEY, JSON.stringify(saved.slice(0, 20)));
  loadSavedRoutes(); toast('Route saved!', 'success');
}
...
function deleteSaved(id) { localStorage.setItem(SAV_KEY, JSON.stringify(getSaved().filter(r => r.id !== id))); loadSavedRoutes(); toast('Removed.', 'info'); }
function rerunSaved(id) { const r = getSaved().find(r => r.id === id); if (!r) return; setVal('o-inp', r.origin); setVal('d-inp', r.dest); APP.originLL = null; APP.destLL = null; toast('Inputs set — click ANALYZE to re-run.', 'info'); }